...
public: // 기본적으로 class는 private 이기 때문에 써줌
...
public:
	...
	MusicStreamingService(string service_name) {
		this->service_name = service_name; //spotify를 넣어라 의미?
	...
	//음악 찾아주는 서비스(title)
	Music* searchByTitle(string title) {               // *은 포인터 *a=1은 주소값 생각,
		for (int i = 0; i < music_list.size(); i++) {
			if (music_list[i].getTitle() == title) {
				return &music_list[i];  // &으로 주소값 반환 or = music_list + i
			}
		}
		return NULL;
	}
	...
	vector<Music*> searchByArtist(string artist) {
		...
		for (int i = 0; i < music_list.size(); i++) {
			if (music_list[i].getArtist() == artist) {
				result.push_back(&music_list[i]);  //&music_list[i] = music_list+i
			}
		}
